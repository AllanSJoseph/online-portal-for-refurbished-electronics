   ...
   <?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        if(isset($_POST["currpassword"]) && isset($_POST["newpassword"])){
            ...
            $isCurrPassValid = $user->validatePassword($uid,$currpasswd); //Returns 0 if the current password is Valid

            if(!$isCurrPassValid){
                $isPassChanged = $user->changePassword($uid,$newpasswd);  //Returns 0 if the Password is changed
                if(!$isPassChanged){
                    echo "<script>";
                ...
                echo 'window.location.href = "buyer_home.php"';
                echo "</script>";
            }
