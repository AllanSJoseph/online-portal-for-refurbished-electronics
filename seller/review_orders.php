...
$orders = $seller->viewOrders($sid);

if (isset($_GET['remove_id'])) {
    $id = intval($_GET['remove_id']);

    $seller->removeProduct($id);
    header('review_orders.php');
}

if (isset($_GET['reviewStatus']) && isset($_GET['oid'])){
    $status = intval($_GET['reviewStatus']);
    $oid = intval($_GET['oid']);
    $seller->reviewProduct($status,$oid);
...
?>
        ...
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav">
              ...
              <li class="nav-item">
                <a class="nav-link active" aria-current="page" href="#">Review Orders</a>
              </li>
              <li class="nav-item">
    ...
    <h2 style="color:DarkSlateGrey;">Review Orders</h2>
        ...
        echo '<table border="1">';
            ...
            foreach($orders as $row){
              ...
              if ($status == NULL){
                ...
                $statusTxt = "<p class=\"text-danger\">SELLER REHECTED</p>";
              }
              echo '<tr>';
                echo '<td>' . $count . '</td>';
                ...
                <button class="btn btn-outline-success" onclick="review(0,'.$row['order_id'].')">Accept</button>
                <button class="btn btn-outline-danger" onclick="review(1,'.$row['order_id'].')">Reject</button></td>';
                echo '</tr>';
...
<div id="myModal" class="modal">
    ...
    <script>
        function review(status,oid){
            if(status == 0){
                if (confirm("Are you sure, you want to Accept this Order?")){
                    window.location.href = '<?php echo $_SERVER['PHP_SELF']; ?>?reviewStatus=0&oid=' + oid;
                }
            }else{
                if (confirm("Are you sure, you want to Reject this Order?")){
                    window.location.href = '<?php echo $_SERVER['PHP_SELF']; ?>?reviewStatus=1&oid=' + oid;
                }
            }
        }
    </script>
